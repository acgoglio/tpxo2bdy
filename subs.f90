...
! reads model value (z/u/v) at given indices (ic,i0,j0) 
! from OTIS-netcdf model files for constituent ic
! output: rc - complex value of field at (ic,i0,j0)
! returns velocities for multi-const netcdf's & TRANSPORTS for atlas
! atlas units: z -mm, u,v -cm^2/sec
!
      subroutine rd_mod_value_da_nc(zuv,type,ncid,ic,i0,j0,ncmod,n,m,rc)
      implicit none
      include 'netcdf.inc'
...
!
      ...
      if(zuv.eq.'z')then
       varname(1)='hRe';varname(2)='hIm'       
      elseif(zuv.eq.'u'.or.zuv.eq.'U')then
       varname(1)='uRe';varname(2)='uIm'
       status=nf_inq_varid(ncid,trim(varname(1)),varid)
       if(status.ne.0)then ! not atlas
        varname(1)='URe';varname(2)='UIm'
       endif         
      else
       varname(1)='vRe';varname(2)='vIm'
       status=nf_inq_varid(ncid,trim(varname(1)),varid)
       if(status.ne.0)then ! not atlas
        varname(1)='VRe';varname(2)='VIm'
       endif   
...
!
      do k=1,2
       status=nf_inq_varid(ncid,trim(varname(k)),varid)
       if(status.ne.0)go to 1
       if(type.eq.'double')then
         status=nf_get_vars_double(ncid,varid,start,count,stride,dtmp)
         if(status.ne.0)go to 2
         rtmp=dtmp
       elseif(type.eq.'real')then
         status=nf_get_vars_real(ncid,varid,start,count,stride,rtmp)
         if(status.ne.0)go to 2
       elseif(type.eq.'int')then
         status=nf_get_vars_int(ncid,varid, &
               start(1:2),count(1:2),stride(1:2),itmp)
         if(status.ne.0)go to 2
         if(zuv.eq.'z')then
           rtmp=1e-3*real(itmp) ! mm->m
         else
           rtmp=1e-4*real(itmp) ! cm^2/sec -> m^2/sec
         endif
       else
        write(*,*)'Error in rd_mod_value_da: unknown type ',trim(type)
       endif
       if(k.eq.1)zre=rtmp
       if(k.eq.2)zim=rtmp
      enddo
      rc=cmplx(zre,zim)
      return
1     write(*,*)'No variable ',trim(varname(k)),' found'
      stop
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rd_inp(modname,c_id,ncon, &
                        APRI,Geo,interp)
      implicit none
      include 'constit.h'
...
2     close(111)
      close(112)      
      write(*,*)'Please check your settings'
...
!
      call rd_mod_file(modname,hname,uname,gname,xy_ll_sub,nca,c_id)
!
      fname=hname
...
!
      ...
      enddo
! read limits from grid file
      call rd_lims_nc(gname,xy_ll_sub,n,m,th_lim,ph_lim,nca)
! 
      if(nca.eq.0)then
       status=nf_inq_varid(ncid,'con',varid)
       status=nf_get_var_text(ncid,varid,c_id(1:nc))
      endif
!
      !write(*,*)n,m,nc
      !write(*,*)th_lim,ph_lim
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rd_mod_header1_nc(fname,n,m,nc,th_lim,ph_lim,c_id)
      implicit none
      include 'netcdf.inc'
      include 'constit.h' 
...
!
      ...
      status=nf_close(ncid)
!
! read limits from same file first (load file)
      ctmp=''
      call rd_lims_nc(fname,ctmp,n,m,th_lim,ph_lim,0)
!
      status=nf_open(trim(fname),nf_nowrite,ncid) 
      status=nf_inq_varid(ncid,'con',varid)
      status=nf_get_var_text(ncid,varid,c_id(1:nc))
...
!
!cc     Interpolates complex array uv(nt,n,m) at point xlat,xlon
!cc     th_lim and ph_lim give latitude and longitude limits of grid
!cc
        implicit none
	...
	real dtheta,dphi,xlonc,x,y,xi,xj,t,u
...
!
         ...
         j0 = int(xj)
         y = xj-j0
!cc      check to see if calculated indices are in range
...
! make_a:: computes A matrix elements for one data point 
	subroutine make_a(interp,ind,nc,time,pu,pf,A,l_sal) 
        implicit none
	include 'constit.h'
!
	...
	real*8 pu(ncmx),pf(ncmx)
        ...
        complex A(nc),c(ncmx)
        integer i,j,nc
! interp - obsolete, now always .false.
...
!
	data index/30,35,19,12,27,17,37,10,25,26,28,33,34, &
                   23,14,24,11,5,3,2,45,46,44,50,0,42,51,40,0,18/
...
! .true. means NO solid Earth correction will be applied in make_a
         call make_a(.false.,ind,ncon,time,pu,pf,A,.true.)
...
!
      ...
      implicit none
      real y,x,lat,lon
      real*8 SLAT,SLON,dx(1,1),dy(1,1),dlon(1,1),dlat(1,1)
      character*1 HEMI
...
!
      ...
      character*1 HEMI
      SLAT=71.;SLON=-70.;HEMI='S'
      dlon(1,1)=lon;dlat(1,1)=lat;
      call mapll(1,1,dlon,dlat,dx,dy,SLAT,SLON,HEMI)
      x=dx(1,1);y=dy(1,1);
      return
      end
!==============================================================
      subroutine fname_atlas(fname,ncid,c_id,nc)
      implicit none
      include 'netcdf.inc'
      include 'constit.h'
      character*4 c_id(ncmx),ctmp
...
!
      icc=1   
! wild card in file name indicate *atlas* 1 constituent files
      k=index(fname,'*')
...
!===============================================================
        subroutine write_z(fname,iunit,z,n,m,nc,th_lim,ph_lim,cid)
        implicit none
        include 'constit.h'
...
!
	...
	write(iunit)n,m,nc,th_lim,ph_lim,cid(1:nc)
        do k=1,nc
         write(iunit)z(k,:,:)
        enddo
        close(iunit)
...
!-------------------------------------------------------------------------
        subroutine write_uv(fname,iunit,u,v,n,m,nc,&
                   th_lim,ph_lim,cid)
        implicit none
        include 'constit.h'
        integer iunit,k,n,m,nc
...
!______________________________________________________________________
!
        subroutine interp_da_nc(ncid,n,m,th_lim,ph_lim,xlat,xlon,uv1, &
                             nc,cind,ierr,zuv,nca)
!
!cc     Interpolates complex fields (z/u/v) at point xlat,xlon
!cc     th_lim and ph_lim -  latitude and longitude limits of grid
!cc     by reading 4 surrounding node values from direct access file
!cc     opened on one unit funit(1), if atals (nca~=0), then on nc units
!cc     returns uv1 - complex, BL interpolated value at xlat, xlon
!cc     Lana, 2020
!
        implicit none
        integer nc,ic,ierr,n,m,k,ic1,nca,cind(nc),nc1
        integer*4 ncid(nc),ncid1
        complex uv1(nc),v00,v10,v01,v11
        real ww(0:1,0:1),dlat,dlon
        real th_lim(2),ph_lim(2),xlon,xlat
	real dtheta,dphi,xlonc,x,y,t,u
        integer iw(0:1),jw(0:1),mask(0:1,0:1)
        real w00,w01,w10,w11,wtot,sm
        character*1 zuv
        character*6 type
!
        if(nca.eq.0)then
...
! check on xlon longitude convention
        xlonc=xlon
! this SHOULD NOT BE DONE for streographic grids! (in km)
! set uv1(1)=-1 in calling routine to skip
! checking lon convention
        if(uv1(1).ne.-1)then
         if(xlonc.lt.ph_lim(1))xlonc=xlonc+360.
         if(xlonc.gt.ph_lim(2))xlonc=xlonc-360.
         uv1(1)=0.
        ...
        dlon=xlonc
!
        call BSI_indices(zuv,dlat,dlon,th_lim,ph_lim, &
                         dphi,dtheta,n,m,x,y,iw,jw)
        nc1=nc
        if(nca.ne.0)nc1=1
        do ic=1,nc
         ic1=cind(ic)
         ncid1=ncid(1)
         if(nca.ne.0)then
            ic1=1 ! atlas 1 constituent files
            ncid1=ncid(ic)
         endif
         v00=0;v10=0;v01=0;v11=0;
         if(ic1.ne.0)then ! no constituent in multi-constituent file
          call rd_mod_value_da_nc(zuv,type,ncid1,ic1,iw(0),jw(0),n,m,nc1,v00)
          call rd_mod_value_da_nc(zuv,type,ncid1,ic1,iw(1),jw(0),n,m,nc1,v10)
          call rd_mod_value_da_nc(zuv,type,ncid1,ic1,iw(0),jw(1),n,m,nc1,v01)
          call rd_mod_value_da_nc(zuv,type,ncid1,ic1,iw(1),jw(1),n,m,nc1,v11)
         endif
         if(ic.eq.1)then
! define 2x2 mask
          mask=0
          if(v00.ne.0)mask(0,0)=1
          if(v10.ne.0)mask(1,0)=1
          if(v01.ne.0)mask(0,1)=1
          if(v11.ne.0)mask(1,1)=1
! find weights
          w00 = (1.-x)*(1.-y)*mask(0,0)
          w01 = (1.-x)*y*mask(0,1)
          w10 = x*(1.-y)*mask(1,0)
          w11 = x*y*mask(1,1)
          wtot = w00+w01+w10+w11
         ...
         uv1(ic)  = v00*ww(0,0)+v10*ww(1,0)+v01*ww(0,1)+v11*ww(1,1)
      enddo
!
      return
      end
...
! bilinear spline interpolation weights
! for delta-forcing
         subroutine BSI_indices(zuv,theta,phi,theta_lim,phi_lim, &
                                dx,dy,n,m,x,y,iw,jw)
         implicit none
         character*1 zuv                ! C-grid zuv: 'u','v','z' ('U','V','Z')
         real theta, phi                ! point coordinates
...
!
         ...
         y = xj-j0
!cc      check to see if calculated indices are in range
         if((i0.gt.n).or.(i0.lt.1).or.(j0.gt.m).or.(j0.lt.1) ) return
!
         j1 = ipshft(j0,1,m)
         i1 = ipshft(i0,1,n)
!
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rd_mod_file(modname,hname,uname,gname,xy_ll_sub,ncatlas,c_id)
      implicit none
      include 'constit.h'
      character*4 c_id(ncmx)
...
5     close(funit)
      ...
      call fname_atlas(hname,funit,c_id,ncatlas)
      return
1     write(*,*)'Check ',trim(modname)
      write(*,*)'1st line can not be empty'
      stop
      end
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rd_grd_nc(gname,nx,ny,hz,mz)
      implicit none
      include 'netcdf.inc'
      character*5 varname
      character*80 gname
...
2     write(*,*)'Error while reading double 2d array ', &
                trim(varname)
      stop
3     write(*,*)'Can not find ',trim(gname)
      stop
4     write(*,*)'Inconsistent dimensions in ',trim(gname)
      stop
      end
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rd_lims_nc(gname,xy_ll_sub,n,m,th_lim,ph_lim,nca)
      implicit none
      include 'netcdf.inc'
...
!
      ...
      if(trim(xy_ll_sub).ne.'')then
        call read2d_double_nc(ncid,'x_z  ',n,m,x)
        call read2d_double_nc(ncid,'y_z  ',n,m,y)
       else
        if(nca.eq.0)then
         call read2d_double_nc(ncid,'lon_z',n,m,x)
         call read2d_double_nc(ncid,'lat_z',n,m,y)
        else
         call read2d_double_nc(ncid,'lon_z',n,1,x(:,1))
         call read2d_double_nc(ncid,'lat_z',1,m,y(1,:))
        endif
      endif
      stx=x(2,1)-x(1,1)
      sty=y(1,2)-y(1,1)
...
!
      ...
      if(status.ne.0)go to 1
      status=nf_get_var_double(ncid,varid,dtmp)
      !status=nf_get_var_double(ncid,varid,dar)
      if(status.ne.0)go to 2
      do k=1,nx
       dar(k,:)=dtmp(:,k)
      enddo
